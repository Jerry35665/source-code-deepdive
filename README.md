...
> **当前进度:五个系列全部完结(151 篇正文 + 111 份调研报告)+ 横向对照 2 篇。**
...
| 横向对照 | 十项目十大横贯模式 + [总目录](cross-series/01-总目录.md) | [2 篇](cross-series/00-横向对照总览.md) | — | ✅ 持续更新 |
| 后续系列 | 候选:Docker / Wireshark / V8 | — | — | 📋 待启动 |
...
1. 每个论断必须落到 `文件:行号`,不确定的写"未核实";与流行说法冲突时以 pin 的 commit 为准并显式指出冲突(历史范例:FFmpeg 的 hls/dash 并非 URLProtocol、`AVSEEK_FLAG_TARGET_TIMESTAMP` 不存在;Git 的 merge-recursive 已整体删除;curl 的 hyper 后端已移除、conncache 已重构为 cpool);
...
- 已计量的 subagent 精确消耗累计约 2.6 亿+(各系列分项见台账)。
...
3. **自研系统借鉴**:按目录选章——转码引擎(FFmpeg 02-04+16)、直播服务(09+17+05)、播放器(10-13+21)、存储引擎(LevelDB 全卷)、数据库内核(PG 三卷)、网络客户端(curl 01+03);
...
- 2026-09-13 ~ 14:第五系列《zstd 深读》(7 篇)、《QuickJS 深读》(7 篇)、《curl 深读》(5 篇)夜间批次交付;全系列总目录索引上线;
- 下一批:后续候选(Docker/Wireshark/V8)与总目录持续维护。
